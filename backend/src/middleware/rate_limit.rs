...
impl RateLimiter {
    ...
    pub async fn cleanup(&self) {
        ...
        // Cleanup IP buckets
        self.ip_buckets
            .retain(|_, (_, start)| now.duration_since(*start) <= self.config.window_duration);
            ...
            if entry.0 >= self.config.max_failures {
                ...
                let backoff = self.config.lockout_duration.as_secs()
                    * (self.config.backoff_multiplier as u64)
                        .pow((entry.0 - self.config.max_failures).min(10));
                ...
                if entry.0 >= self.config.max_failures {
                    let backoff = self.config.lockout_duration.as_secs()
                        * (self.config.backoff_multiplier as u64)
                            .pow((entry.0 - self.config.max_failures).min(10));
                    let lockout =
                        Duration::from_secs(backoff.min(self.config.max_lockout.as_secs()));
                    entry.1 = Some(now + lockout);
