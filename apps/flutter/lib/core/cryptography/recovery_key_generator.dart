...
/// P0-3: Recovery key hardening - 128-bit entropy minimum
/// Replaces the weak 6-character recovery key with a secure mnemonic phrase.

/// BIP39 English wordlist (first 2048 words) - truncated for brevity
/// In production, use a full BIP39 wordlist package.
const _wordlist = [
  ...
  'desk', 'despair', 'destroy', 'detect', 'develop', 'device', 'devote', 'diagram',
  ...
  'junior', 'junk', 'just', 'kangaroo', 'keep', 'kelp', 'kettle', 'kick',
  ...
  'picture', 'piece', 'pig', 'pigeon', 'pilot', 'pin', 'pine', 'pink',
  ...
  'vanish', 'vapor', 'various', 'vein', 'vendor', 'venture', 'venue', 'verb',
...
/// P0-3: Generate a secure recovery phrase with 128 bits of entropy
/// Returns a 12-word mnemonic phrase (BIP39-style)
String generateRecoveryPhrase({int entropyBits = 128}) {
  if (entropyBits < 128) {
    throw ArgumentError('Entropy must be at least 128 bits for security');
  }

  final random = Random.secure();
  final byteCount = entropyBits ~/ 8;
  ...
  // Convert bytes to word indices (11 bits per word for 2048 wordlist)
  final words = <String>[];
  var bitIndex = 0;
  final totalBits = byteCount * 8;
  
  // Simple approach: use 12 words for 128 bits (11 bits per word = 132 bits)
  // This gives us 12 words with some padding
  for (int i = 0; i < 12; i++) {
    // Extract 11 bits for each word index
    int wordIndex = 0;
    for (int j = 0; j < 11 && bitIndex < totalBits; j++) {
      final byteIndex = bitIndex ~/ 8;
      final bitOffset = bitIndex % 8;
      final bit = (bytes[byteIndex] >> (7 - bitOffset)) & 1;
      wordIndex |= (bit << (10 - j));
      bitIndex++;
    }
    // Mask to ensure we're in range [0, 2047]
    wordIndex &= 0x7FF;
    words.add(_wordlist[wordIndex]);
  }

  return words.join(' ');
}

/// P0-3: Validate a recovery phrase
/// Returns true if all words are valid and the phrase has correct structure
bool validateRecoveryPhrase(String phrase) {
  final words = phrase.trim().toLowerCase().split(RegExp(r'\s+'));
  
  // Must have 12 words for 128-bit entropy
  if (words.length != 12) {
    return false;
  }
...
/// P0-3: Generate a high-entropy recovery key as base64
/// Alternative to mnemonic phrase - 22 characters of base64 = 128 bits
String generateRecoveryKeyBase64({int entropyBits = 128}) {
  if (entropyBits < 128) {
    throw ArgumentError('Entropy must be at least 128 bits for security');
  }
  ...
  // Base64 encode and remove padding for user convenience
  return base64Url.encode(bytes).replaceAll('=', '');
}

/// Legacy function - DEPRECATED
/// @deprecated Use [generateRecoveryPhrase] instead for 128-bit entropy
@Deprecated('Use generateRecoveryPhrase() for secure 128-bit entropy')
String generateRecoveryKey({int length = 6}) {
  // This is the old weak implementation - kept for backward compatibility only
  // DO NOT USE - only 6 characters = ~31 bits of entropy = trivially brute-forced
